...
class AttributeItem:
	...
	def __init__(self, buffer, offset=0):
		...
		if self._itemTuple[2] > 0:
			self._name=(buffer[offset+self._itemTuple[3]:
						offset+self._itemTuple[3]+2*self._itemTuple[2]])
		else:
			self._name=None
	...
	def __init__(self, buffer, offset=0):
		...
		for i in range(0, self._headerTuple[2]):
		   ...
		   self._data = (self._data[0: replaceIndex] + 
		   					self._data[42+i*2:42+i*2+2] + 
		   					self._data[replaceIndex + 2: ])
		# now read the entries
		pos=offset+self._headerTuple[5]
		self._entries=[]
		while pos < offset + self._headerTuple[6] + 24:
...
class Bitmap(Attribute):
	...
	def __init__(self, buffer, offset):
		...
		self._bitmap=(buffer[offset+self.attributeOffset():
		offset+self.attributeOffset()+self.attributeLength()])
	...
	def __str__(self):
		retStr=Attribute.__str__(self)
		retStr+=('\nClusters in use/bitmap: ' + 
					str(self.clustersInUse()) + '/' + 
					str(self.clustersInMap()) )
		return retStr
